...
class ProxyImplTest {
    ...
    fun setup() {
        ...
        shortcutCreatorProxy = ShortcutCreatorProxyImpl(context, getActivityIconUseCase)
    ...
    fun testActivityLauncherProxySendsIntent() {
        ...
        argumentCaptor<Intent>().apply {
            ...
            assertEquals(componentName.flattenToString(), intent.getStringExtra(ActivityLauncherProxy.INTENT_EXTRA_COMPONENT))
            val capturedExtras = intent.getBundleExtra(ActivityLauncherProxy.INTENT_EXTRA_EXTRAS)
            assertEquals("b", capturedExtras?.getString("a"))
        }
    }
